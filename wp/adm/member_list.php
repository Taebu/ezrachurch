...
$rows = $config['cf_page_rows'];
    ...
    $_SESSION["listsize"]=$listsize;
$rows = $_SESSION["listsize"];
$rows = $listsize;
}
...
$list_ezra = '<a href="'.$_SERVER['SCRIPT_NAME'].'?ezrachurch=on" class="ov_listall">서울에스라교회</a>';

$list_sebh = '<a href="'.$_SERVER['SCRIPT_NAME'].'?sehb=on" class="ov_listall">에스라성서원</a>';
$g5['title'] = '회원관리';
...
<label>
<input type="checkbox" name="ezra" onclick="$('#fsearch').submit();" 
<?php echo $_GET['ezra']=='on'?'checked':''; ?>>서울에스라교회</label>
<label>
<input type="checkbox" name="sebh"  onclick="$('#fsearch').submit();" 
<?php echo $_GET['sebh']=='on'?'checked':''; ?>>에스라성서원</label>
<input type="submit" class="btn_submit" value="검색">
    <select name="listsize">
    <option value="15"<?php echo get_selected($_GET['listsize'], "15"); ?>>15</option>
    <option value="30"<?php echo get_selected($_GET['listsize'], "30"); ?>>30</option>
    <option value="50"<?php echo get_selected($_GET['listsize'], "50"); ?>>50</option>
    <option value="100"<?php echo get_selected($_GET['listsize'], "100"); ?>>100</option>
    <option value="200"<?php echo get_selected($_GET['listsize'], "200"); ?>>200</option>
    <option value="1000"<?php echo get_selected($_GET['listsize'], "1000"); ?>>1000</option>
    <option value="2000"<?php echo get_selected($_GET['listsize'], "2000"); ?>>2000</option>
    </select>
</form>
...
<div class="tbl_head02 tbl_wrap">
    ...
    <tr>
        ...
        <th scope="col" id="mb_list_id" rowspan="2"><?php echo subject_sort_link('mb_id') ?>회원 구분</a></th>
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++) {
		...
		if($row['mb_level'] == '2') $mb_level = "에스라성서원";
		else if($row['mb_level'] >= '3') $mb_level = "서울에스라교회";
    ...
    <tr class="<?php echo $bg; ?>">
        ...
        <td headers="mb_list_id" class="td_name sv_use" rowspan="2"><?php echo $mb_level; ?></td>
        <td headers="mb_list_id" class="td_name sv_use" rowspan="2"><?php echo $mb_id ?></td>
...
echo get_paging(G5_IS_MOBILE ? $config['cf_mobile_pages'] : $config['cf_write_pages'], $page, $total_page, '?'.$qstr.'&amp;page=',$param);
$list_size=G5_IS_MOBILE ? $config['cf_mobile_pages'] : $config['cf_write_pages'];
 ?>
