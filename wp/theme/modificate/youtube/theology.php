...
?>
 ...
 <script src="/wp/theme/modificate/youtube/js/edu_01.js"></script>
...
function get_youtube(q)
{
  $("#btn_more").hide();
	...
	is_same_keyword=old_keyword==q;
			...
			{		/* 시편 찬송가가 아닌 경우 */
			console.log(ez_youtube_wm[i]);
		if(!is_psalter&&ez_youtube_wm[i].ey_title.replace(/ /g,"").indexOf(q)>-1)
		{
...
//		if(is_psalter&&ez_youtube_wm[i].ey_title.indexOf(q)>-1&&ez_youtube_wm[i].ey_title.indexOf("칼빈")>-1)
	...
	for(var i in bible_title)
	{
	  ...
	  {
		  console.log(ez_youtube_wm[j].ey_title.replace(/ /g,""));
		if(ez_youtube_wm[j].ey_title.replace(/ /g,"").indexOf(bible_title[i])>-1)cnt++;
		total_cnt++;
	  }
...
?>
<a id="book_0" href="javascript:get_layout('intro');" class="list-group-item">소개</a>
<a id="book_1" href="javascript:get_layout('history');" class="list-group-item">연혁</a>
<a id="book_2" href="javascript:get_layout('law');" class="list-group-item">회칙</a>
<a id="book_3" href="javascript:get_layout('donation');" class="list-group-item">후원단체</a>
  <?php } /* 에스겔 주일 대하설교 */?>
</div>
...
<div id="youtube_area">#youtube_area</div>
