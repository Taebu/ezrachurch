...
for($i =0; $rss = mysql_fetch_array($querys); $i++){
	if($i == 0){ 
		$sql .= "SELECT * FROM ";
		$sql .= "(SELECT wr_subject,wr_id,wr_datetime,wr_parent,wr_is_comment,wr_name,ca_name,wr_content, '$rss[bo_table]' AS bo_table,wr_hit, '$rss[bo_subject]' AS bo_subject FROM g4_write_$rss[bo_table] "; 
	}else{ 
		$sql .= "UNION ALL SELECT wr_subject,wr_id,wr_datetime,wr_parent,wr_is_comment,wr_name,ca_name,wr_content, '$rss[bo_table]' AS bo_table,wr_hit, '$rss[bo_subject]' AS bo_subject FROM g4_write_$rss[bo_table] "; 
	} 
} 
if($type=="comment"){
//$sql .= ") a where a.wr_is_comment = '1' $where order by a.wr_datetime desc limit $firstNum,$listsize"; 
$sql .= ") a where a.wr_is_comment = '1' $where order by a.wr_datetime desc"; 
}else{
//$sql .= ") a where a.wr_is_comment = '0' $where  and a.wr_subject!=''  order by a.wr_datetime desc limit $firstNum,$listsize";
$sql .= ") a where a.wr_is_comment = '0' $where  and a.wr_subject!=''  order by a.wr_datetime desc"; 
}
...
for($i =0; $rss = mysql_fetch_array($querys); $i++){
	if($i == 0){ 
		...
		$sql .= "(SELECT wr_subject,wr_id,wr_datetime,wr_parent,wr_is_comment,wr_name,ca_name,wr_content, '$rss[bo_table]' AS bo_table, '$rss[bo_subject]' AS bo_subject FROM g4_write_$rss[bo_table] "; 
	}else{ 
		$sql .= "UNION ALL SELECT wr_subject,wr_id,wr_datetime,wr_parent,wr_is_comment,wr_name,ca_name,wr_content, '$rss[bo_table]' AS bo_table, '$rss[bo_subject]' AS bo_subject FROM g4_write_$rss[bo_table] "; 
	} 
} 
if($type=="comment"){
...
//	$product['wr_datetime']=date("m-d",strtotime($rs[wr_datetime]));
	$product['wr_datetime']=$rs['wr_datetime'];
...
$new_jsfile="/*".date("Y-m-d H:i:s")."*/var nb_comment=".$string_json.";";


$file=fopen("new_comment.js","w");
