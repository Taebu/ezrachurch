...
<style>
body{padding: 15px;font-family: "lee_sun_shin_dotum_b";font-size: 32px}
...
if($pre)
{
	printf("<a href='./view.php?eh_no=%s' class='btn btn_lg btn_primary'>이전</a>",$pre['eh_no']);
}else{	
...
if($ord)
{
	printf("<a href='./view.php?eh_no=%s' class='btn btn_lg btn_primary'>다음</a>",$ord['eh_no']);
}else{	
...
echo sprintf("<input type='hidden' name='eh_no' value='%s'>",$view['eh_no']);
echo "<input type='hidden' name='table' value='westminster_confession'>";
echo '보드번호 : ';
echo $view['eh_no'];
echo '<hr>';
echo $view['eh_chapter'];echo '장';
//echo "&nbsp;".$SUBJECT['ko']["제".$view['eh_chapter']."장"];
echo '(';
//echo $SUBJECT['en']["제".$view['eh_chapter']."장"];
echo ')';
echo '<hr>';
echo "".$view['eh_clause'];echo '항&nbsp;';
echo $view['eh_subject'];
echo '<hr>';	
echo '내용 : ';
echo '<hr>';
echo "<div class='westminster_blacktheme'>";
//echo preg_replace("/(\d+)\)/", "<sup>$1)</sup>", nl2br($view['eh_content']));;
echo  nl2br($view['eh_content']);;
echo '</div><!-- .westminster_blacktheme -->';
echo '<hr>';
echo '내용(영문) : ';
echo '<hr>';
echo nl2br($view['eh_content_eng']);
echo '<hr>';	

echo '관련구절 : ';
echo '<hr>';
echo "<div class='westminster_blacktheme'>";
echo nl2br($view['eh_relparse']);
echo '</div><!-- .westminster_blacktheme -->';
echo '<hr>';
echo "</div>";
echo '관련구절 (영문) : ';
echo '<hr>';
echo nl2br($view['eh_relparse_eng']);
echo '<hr>';

echo '해설 : ';
echo '<hr>';
echo nl2br($view['eh_commentary']);
echo '<hr>';	
echo '해설 (영문) : ';
echo '<hr>';
echo nl2br($view['eh_commentary_eng']);
echo '<hr>';
...
print("<a class='btn btn_lg btn_danger fontBig'>크게</a>");
if(!$is_block_ip)
echo sprintf("<input  class='btn btn_lg btn_primary' type='button' value='수정' onclick=\"location.href='./modify.php?eh_no=%s'\">",$view['eh_no']);
